...
> 除以 $\sqrt{d}$ 的目的是为了防止梯度消失。如果不除以这个数，随着维度 $d$ 的增大，点积的结果会变得非常大，导致 Softmax 函数进入“饱和区”，使得梯度几乎为 0，模型无法训练。
...
![掩码矩阵](https://raw.githubusercontent.com/Flower-Melon/image/main/img/2025/掩码矩阵.png)
...
> 看了这么长时间的`transformer`了，没想到还是对数据流有一定的认知误区，这是不应该的

之前的认知里，我认为每生成一个`token`都要计算整个句子的`Q K V`（和训练时一样），实际上：

$$Output_{t+1} = \text{Softmax}( \mathbf{q_{t+1}} \cdot [k_1, k_2, ..., k_{t+1}]^T ) \cdot V_{1:t+1}$$

实际上计算`next token`的`q`是上一个`token`，`K V`是整个句子的，因此需要设计`KV cache`来储存之前词元对应的`K V`，以省去每一次不必要的计算

从这个角度来看，`QKV`的设计是有其意义的，并非我之前理解的没有本质不同